...
def abort_if_empty(yaml_content: str, file: Path, kind: FileKind) -> None:
    """Abort when `file` holds no YAML for the server to act on."""
    if yaml_content:
        return
    abort(f"{_format_file_label(kind, file)} contains no YAML content.", code=ErrorCode.INVALID_INPUT)
